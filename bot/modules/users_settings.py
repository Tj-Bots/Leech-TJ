...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        buttons.ibutton("◀️", f"userset {user_id} back", "footer", style="bg_primary")
        buttons.ibutton("❌", f"userset {user_id} close", "footer", style="bg_danger")
        button = buttons.build_menu(2)
    elif key == "leech":
        if (
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        await query.answer()
        buttons = ButtonMaker()
        buttons.ibutton("❌", f"wzmlx {user_id} close")
        ...
        await query.answer()
        buttons = ButtonMaker()
        buttons.ibutton("✓", f"userset {user_id} reset_now y")
        buttons.ibutton("✘", f"userset {user_id} reset_now n")
        buttons.ibutton("❌", f"userset {user_id} close", "footer")
        await editMessage(
